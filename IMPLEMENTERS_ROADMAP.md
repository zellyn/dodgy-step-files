...
**109 fixtures hand the reader a complete, connected B-rep and get an empty shape back.** Not a partial result, not a repaired face — nothing, and no error. This is the worst outcome in the corpus, because the import reports success and the user's geometry is simply gone.
...
5. **Wrong argument count on core geometry entities** (11 fixtures) — `VECTOR(#2,100.)`: the name omitted, two arguments where three are declared, so every later value sits in the wrong slot. Same positional mechanism as the B-spline case in check 2, on the small entities.
