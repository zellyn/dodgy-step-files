...
Catalog claim: Sender wrote area/volume/centroid validation properties
that do not match what the receiver computes from the geometry within
agreed tolerance. Triggers include round-trip drift, degenerated torus
reporting overlarge bbox, and non-mm export units mismatching geometry.

Reproducer recipe: 1 mm cube with GEOMETRIC_VALIDATION_PROPERTY carrying
volume=2.0, area=12.0 (double the truth: volume=1.0, area=6.0). Receiver
recomputes and the values disagree beyond tolerance.

Byte assertions:
  (none — defect is purely in the GVP values, not entity vocabulary)
...
f = StepFile(
    ...
    schema="AP214",
    defect=(
        "Validation properties value mismatch (geometry self-check); "
        "input: AP214 STEP file with MANIFOLD_SOLID_BREP (1 mm cube) where "
        "GEOMETRIC_VALIDATION_PROPERTY carries volume=2.0 and area=12.0 — "
        "double the true values (volume=1.0, area=6.0 for a 1 mm cube); "
        "receiver recomputes from the geometry and finds volume=1.0, area=6.0; "
        "disagreement exceeds agreed tolerance; "
        "triggers: round-trip drift, degenerated torus reporting overlarge bbox, "
        "non-mm export units mismatching geometry (G101); "
        "kernel must emit 'validation property check failed' diagnostic; "
        "archive systems must reject on failure; CAD-to-CAD must warn; "
        "synonyms: validation property fails check, GVP volume disagrees with computed, "
        "round-trip volume drift, STEP claimed volume disagrees with computed, "
        "geometry self-check fails"
    ),
)


# ── Cube geometry at fixed IDs (shared pattern with M008/M011/M014) ──────────
_CUBE_BODY = """\
...
def _render_m026() -> str:
    lines = []
    lines.append("ISO-10303-21;")
    lines.append(f"/* M026: {f.defect} */")
    lines.append("HEADER;")
    lines.append("FILE_DESCRIPTION(('M026'),'2;1');")
    lines.append("FILE_NAME('M026.stp','2026-06-21T00:00:00',(''),(''),'cad-research-suite','','');")
    ...
    lines.append("/* Defect: GVP volume=2.0 and area=12.0 are double the true values.")
    lines.append("   True 1mm cube: volume=1.0 mm^3, area=6.0 mm^2.")
    lines.append("   Receiver recomputes and finds disagreement beyond tolerance. */")
    lines.append("#600=SHAPE_ASPECT('cube_shape','',#350,.T.);")
    lines.append("#610=PROPERTY_DEFINITION('geometric validation property','volume GVP mismatch',#600);")
    lines.append("#620=MEASURE_REPRESENTATION_ITEM('volume',VOLUME_MEASURE(2.0),#10);")
    lines.append("#621=REPRESENTATION('',(#620),#14);")
    lines.append("#622=PROPERTY_DEFINITION_REPRESENTATION(#610,#621);")
    lines.append("#630=PROPERTY_DEFINITION('geometric validation property','area GVP mismatch',#600);")
    lines.append("#640=MEASURE_REPRESENTATION_ITEM('area',AREA_MEASURE(12.0),#10);")
    lines.append("#641=REPRESENTATION('',(#640),#14);")
    lines.append("#642=PROPERTY_DEFINITION_REPRESENTATION(#630,#641);")
    lines.append("#650=CARTESIAN_POINT('centroid_wrong',(1.0,1.0,1.0));")
    lines.append("#660=PROPERTY_DEFINITION('geometric validation property','centroid GVP mismatch',#600);")
    lines.append("#661=REPRESENTATION('',(#650),#14);")
    lines.append("#662=PROPERTY_DEFINITION_REPRESENTATION(#660,#661);")
    lines.append("ENDSEC;")
    lines.append("END-ISO-10303-21;")
    return "\n".join(lines) + "\n"


def _write_m026(path) -> None:
    _Path(path).write_text(_render_m026())


f.render = _render_m026   # type: ignore[method-assign]
f.write  = _write_m026    # type: ignore[method-assign]
