...
Catalog claim: When an assembly is split using External References, the PDM/structure
file holds part identity but not geometry. If GVPs were attached only to shape_aspects
of solids/faces, they live in the geometry file and are unreachable from the structure
file.

Reproducer recipe: STEP file with a MANIFOLD_SOLID_BREP (1 mm cube) and a GVP for
volume attached only to a SHAPE_ASPECT of the solid — not at part/product level.
PDM-only readers that consume only the structure file see no GVP.
...
f = StepFile(
    ...
    schema="AP214",
    defect=(
        "Validation properties only at geometry level, invisible to PDM-only readers; "
        "input: AP214 STEP file with MANIFOLD_SOLID_BREP (1 mm cube) where the volume "
        "GVP is attached only to a SHAPE_ASPECT of the solid, not at product level; "
        "when an external-reference assembly is split, PDM-only readers that consume "
        "only the structure file cannot see the GVP; "
        "writers must always emit GVPs at part/product level too; "
        "kernel must heal and accept: warn that product-level GVP is missing; "
        "synonyms: GVP invisible to PDM reader, validation properties only at geometry level, "
        "structure file misses validation property, external-reference PDM has no GVP"
    ),
)


# ── Cube geometry shared by M008/M011/M014 ────────────────────────────────────
_CUBE_BODY = """\
/* 8 corner points */
...
def _render_m008() -> str:
    lines = []
    lines.append("ISO-10303-21;")
    lines.append(f"/* M008: {f.defect} */")
    lines.append("HEADER;")
    lines.append("FILE_DESCRIPTION(('M008'),'2;1');")
    lines.append("FILE_NAME('M008.stp','2026-06-21T00:00:00',(''),(''),'cad-research-suite','','');")
    lines.append("FILE_SCHEMA(('AUTOMOTIVE_DESIGN { 1 0 10303 214 3 1 1 }'));")
    lines.append("ENDSEC;")
    lines.append("DATA;")
    lines.append("/* Units and context */")
    ...
    lines.append("/* Cube solid (MANIFOLD_SOLID_BREP referencing #510=CLOSED_SHELL) */")
    lines.append("#500=MANIFOLD_SOLID_BREP('cube_solid',#510);")
    lines.append(_CUBE_BODY)
    lines.append("/* CLOSED_SHELL */")
    ...
    lines.append("/* SHAPE_ASPECT on the solid — GVP attaches here only, NOT at product level.")
    lines.append("   PDM-only readers consuming the structure file will miss this GVP. */")
    lines.append("#540=SHAPE_ASPECT('cube_solid_aspect','',#350,.T.);")
    lines.append("/* GVP: volume only at geometry/shape_aspect level */")
    lines.append("#600=PROPERTY_DEFINITION('geometric validation property','volume',#540);")
    lines.append("#610=MEASURE_REPRESENTATION_ITEM('volume',VOLUME_MEASURE(1.0),#10);")
    lines.append("#620=REPRESENTATION('',(#610),#14);")
    lines.append("#630=PROPERTY_DEFINITION_REPRESENTATION(#600,#620);")
    lines.append("ENDSEC;")
    lines.append("END-ISO-10303-21;")
    return "\n".join(lines) + "\n"


def _write_m008(path) -> None:
    _Path(path).write_text(_render_m008())


f.render = _render_m008   # type: ignore[method-assign]
f.write  = _write_m008    # type: ignore[method-assign]
