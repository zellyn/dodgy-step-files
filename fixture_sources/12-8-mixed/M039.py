...
Catalog claim: Inventor 2014 emits STYLED_ITEM entities not parented under
MECHANICAL_DESIGN_GEOMETRIC_PRESENTATION_REPRESENTATION or DRAUGHTING_MODEL,
so OCCT skips them and drops all colors.

Reproducer recipe: 1 mm cube MANIFOLD_SOLID_BREP with two bare STYLED_ITEM
entities referencing cube faces but not placed under any MDGPR or
DRAUGHTING_MODEL wrapper.
...
f = StepFile(
    ...
    schema="AP214",
    defect=(
        "STYLED_ITEM not parented under MDGPR/DRAUGHTING_MODEL (Autodesk Inventor 2014); "
        "input: AP214 STEP file with MANIFOLD_SOLID_BREP (1 mm cube) where two "
        "STYLED_ITEM entities reference cube faces with colour bindings but are "
        "not placed inside any "
        "MECHANICAL_DESIGN_GEOMETRIC_PRESENTATION_REPRESENTATION or "
        "DRAUGHTING_MODEL wrapper; "
        "OCCT ec661e4 (Mantis 0024159): reader skipped bare STYLED_ITEMs, "
        "dropping all colors on import; "
        "kernel must heal: accept and apply styles even without a "
        "presentation-representation parent; "
        "synonyms: STYLED_ITEM not under MDGPR, Inventor styled_item dropped, "
        "presentation parent missing, all colors lost on Inventor STEP"
    ),
)
...
def _render_m039() -> str:
    lines = []
    lines.append("ISO-10303-21;")
    lines.append(f"/* M039: {f.defect} */")
    lines.append("HEADER;")
    lines.append("FILE_DESCRIPTION(('M039'),'2;1');")
    lines.append("FILE_NAME('M039.stp','2026-06-21T00:00:00',(''),(''),'cad-research-suite','','');")
    ...
    lines.append("/* Defect: bare STYLED_ITEMs with no MDGPR/DRAUGHTING_MODEL parent.")
    lines.append("   Autodesk Inventor 2014 emits STYLED_ITEM at the top level.")
    lines.append("   OCCT ec661e4 (Mantis 0024159) silently skips them — all colors lost.")
    lines.append("   Both STYLED_ITEMs reference a real face (#5850) with a COLOUR_RGB. */")
    lines.append("#600=COLOUR_RGB('inventor_red',0.8,0.1,0.1);")
    lines.append("#601=FILL_AREA_STYLE_COLOUR('',#600);")
    lines.append("#602=FILL_AREA_STYLE('',(#601));")
    lines.append("#603=SURFACE_STYLE_FILL_AREA(#602);")
    lines.append("#604=SURFACE_SIDE_STYLE('',(#603));")
    lines.append("#605=SURFACE_STYLE_USAGE(.BOTH.,#604);")
    lines.append("#606=PRESENTATION_STYLE_ASSIGNMENT((#605));")
    lines.append("/* Bare STYLED_ITEM: not inside MDGPR or DRAUGHTING_MODEL. Inventor bug. */")
    lines.append("#607=STYLED_ITEM('bottom_face_style',(#606),#5850);")
    lines.append("#610=COLOUR_RGB('inventor_blue',0.1,0.1,0.8);")
    lines.append("#611=FILL_AREA_STYLE_COLOUR('',#610);")
    lines.append("#612=FILL_AREA_STYLE('',(#611));")
    lines.append("#613=SURFACE_STYLE_FILL_AREA(#612);")
    lines.append("#614=SURFACE_SIDE_STYLE('',(#613));")
    lines.append("#615=SURFACE_STYLE_USAGE(.BOTH.,#614);")
    lines.append("#616=PRESENTATION_STYLE_ASSIGNMENT((#615));")
    lines.append("#617=STYLED_ITEM('top_face_style',(#616),#5851);")
    lines.append("ENDSEC;")
    lines.append("END-ISO-10303-21;")
    return "\n".join(lines) + "\n"


def _write_m039(path) -> None:
    _Path(path).write_text(_render_m039())


f.render = _render_m039   # type: ignore[method-assign]
f.write  = _write_m039    # type: ignore[method-assign]
