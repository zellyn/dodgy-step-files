...
Catalog claim: Supplemental geometry referenced by PMI geometric_item_specific_usage
must use the constructive_geometry_representation as used_representation. If the writer
uses the part's shape_representation instead, the PMI link is invalid.

Reproducer recipe: PMI element references a supplemental plane via GISU whose
used_representation is the ADVANCED_BREP_SHAPE_REPRESENTATION of the manufactured
shape, not the CGR.
...
f = StepFile(
    ...
    schema="AP242",
    defect=(
        "Anonymous CGR relationship breaks PMI link; "
        "input: AP242 STEP file with MANIFOLD_SOLID_BREP (1 mm cube) plus "
        "CONSTRUCTIVE_GEOMETRY_REPRESENTATION containing a datum plane; "
        "PMI annotation uses GEOMETRIC_ITEM_SPECIFIC_USAGE (GISU) to reference "
        "the datum plane, but GISU.used_representation points at the "
        "ADVANCED_BREP_SHAPE_REPRESENTATION (the manufactured shape) instead of the CGR; "
        "Supplemental Geometry §5.4: GISU.used_representation must be the CGR "
        "that contains the supplemental element; "
        "kernel must reject the broken link with diagnostic; "
        "OCC loads the shape without diagnostic (reads as healing); "
        "synonyms: anonymous CGR relationship breaks PMI, GISU links wrong representation, "
        "PMI link to part shape instead of CGR, supplemental geometry PMI binding lost"
    ),
)
...
def _render_m014() -> str:
    lines = []
    lines.append("ISO-10303-21;")
    lines.append(f"/* M014: {f.defect} */")
    lines.append("HEADER;")
    lines.append("FILE_DESCRIPTION(('M014'),'2;1');")
    lines.append("FILE_NAME('M014.stp','2026-06-21T00:00:00',(''),(''),'cad-research-suite','','');")
    ...
    lines.append("#604=PLANE('datum_A',#603);")
    lines.append("/* CGR that properly owns the datum plane */")
    lines.append("#620=CONSTRUCTIVE_GEOMETRY_REPRESENTATION('cgr_datums',(#604),#14);")
    lines.append("/* GISU — Defect: used_representation points at #520 (the ABREP shape) */")
    lines.append("/* Correct: used_representation should be #620 (the CGR) */")
    lines.append("/* Kernel must check that GISU.used_representation actually contains #604 */")
    lines.append("#700=GEOMETRIC_ITEM_SPECIFIC_USAGE('pmi_plane_link','pmi link to datum A',")
    lines.append("  #604,")
    lines.append("  #520);")
    lines.append("/* #520 is the ADVANCED_BREP_SHAPE_REPRESENTATION: it does NOT contain #604.")
    lines.append("   A conformant reader must reject this broken PMI link. */")
    lines.append("ENDSEC;")
    lines.append("END-ISO-10303-21;")
    return "\n".join(lines) + "\n"


def _write_m014(path) -> None:
    _Path(path).write_text(_render_m014())


f.render = _render_m014   # type: ignore[method-assign]
f.write  = _write_m014    # type: ignore[method-assign]
