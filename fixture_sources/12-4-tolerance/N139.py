...
Nested wire tolerance scaling without recursion depth tracking. Circular wire
references with tolerance propagation have no cycle counter; tolerance
validation chains cause stack exhaustion or infinite loops.

Reproducer: face with two edges (ec1, ec2) forming a closed loop; OuterWire
tolerance traversal on nested wire structure lacks depth limit and cycle
detection.
...
f = StepFile(
    catalog_id="N139",
    defect=(
        "CLOSED_SHELL with face containing ec1 (v1→v2) and ec2 (v3→v4) in nested "
        "wire; OuterWire tolerance traversal lacks recursion depth limit and cycle "
        "detection — circular wire tolerance propagation causes stack exhaustion"
    ),
)
...
f.write(Path(__file__).parent.parent.parent / "step-examples" / "12-4-tolerance" / "N139.stp")
