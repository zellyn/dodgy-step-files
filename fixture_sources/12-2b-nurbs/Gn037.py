...
Catalog claim: A B_SPLINE_CURVE_WITH_KNOTS with legitimate poles but a knot
vector that collapses to one unique value (knots=(0.,0.) with multiplicities
(4,4)). The unique-knot count is 1, so the parametric range is empty and
...
STEP mechanism (literal):
  - B_SPLINE_CURVE_WITH_KNOTS degree 3, 4 control points.
    Knots: (0.0, 0.0) mults (4, 4) — sum=8 but both unique values are 0.0:
    the parametric domain is [0.0, 0.0] — an empty range.
    A compliant reader must reject with a diagnostic; OCC silently accepts.
  - C-1 break in 3D edge (1.5-unit CP gap at t=0.5) drives shape_null=True.

Mechanism vs driver:
  - CATALOG MECHANISM: B_SPLINE_CURVE_WITH_KNOTS knots=(0.0,0.0) mults=(4,4);
    only one unique knot value; parametric range collapses to empty; compliant
    reader must reject with diagnostic; OCC silently accepts. See Gn001, Gn003.
...
f = StepFile(
    ...
    defect=(
        "B_SPLINE_CURVE_WITH_KNOTS degree 3, 4 control points; "
        "knots=(0.0,0.0) mults=(4,4): only one unique knot value; "
        "parametric range collapses to empty [0.0,0.0]; compliant reader must "
        "reject with diagnostic naming empty parametric domain; OCC silently "
        "accepts; See also Gn001, Gn003; "
...
# ── CATALOG MECHANISM: collapsed knot vector ──────────────────────────────────
# Degree 3, 4 control points => need sum(mults)=degree+ncp+1=3+4+1=8.
# (4,4) sums to 8 ✓ in count, but both unique values = 0.0 → empty domain.
cp0 = f.cartesian_point((0.0, 0.0, 0.0))
cp1 = f.cartesian_point((2.0, 0.0, 0.0))
cp2 = f.cartesian_point((3.0, 0.0, 0.0))
cp3 = f.cartesian_point((5.0, 0.0, 0.0))

# THE DEFECT: both unique knot values are 0.0 — empty parametric range.
collapsed_curve = f._emit_raw(
    f"B_SPLINE_CURVE_WITH_KNOTS('collapsed_knots',3,"
    f"(#{cp0.eid},#{cp1.eid},#{cp2.eid},#{cp3.eid}),"
    f".UNSPECIFIED.,.F.,.F.,"
    f"(4,4),(0.0,0.0),.UNSPECIFIED.)"
)
...
bspline_3d = f._emit_raw(
    f"B_SPLINE_CURVE_WITH_KNOTS('gn037_c1_break',2,"
...
pc_bspline = f._emit_raw(
    f"B_SPLINE_CURVE_WITH_KNOTS('gn037_pc',2,"
...
defrep = f._emit_raw(
    f"DEFINITIONAL_REPRESENTATION('gn037_pcdef',(#{pc_bspline.eid}),#{prc.eid})"
)
pcurve = f._emit_raw(f"PCURVE('gn037_pc_ent',#{surf.eid},#{defrep.eid})")
sc = f._emit_raw(
    f"SURFACE_CURVE('gn037_sc',#{bspline_3d.eid},(#{pcurve.eid}),.PCURVE_S1.)"
)
...
e_defect = f._emit_raw(
    f"EDGE_CURVE('gn037_edge',#{v_a.eid},#{v_b.eid},#{sc.eid},.T.)"
)
