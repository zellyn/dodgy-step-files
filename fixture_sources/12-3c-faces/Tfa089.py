...
Catalog claim: Face slightly larger than the spot threshold (0.1 × 0.1 square)
but with vertex tolerances (0.05) larger than the face dimension. Tolerance-scaled
spot comparison incorrectly misclassifies as spot face.

Mechanism: CLOSED_SHELL box with a tiny top face 0.12 × 0.12 mm (just above the
0.1 spot threshold). All four top-face vertices carry tolerance 0.05 (larger than
half the face dimension). The spot detector computes a tolerance-scaled threshold
that inflates the comparison value, making 0.12 appear smaller than the inflated
threshold and incorrectly classifying it as a spot face.
...
f = StepFile(
    catalog_id="Tfa089",
    defect=(
        "CLOSED_SHELL: top face 0.12×0.12 mm (just above 0.1 spot threshold); "
        "all four top-face vertices have tolerance=0.05 (> face half-dimension); "
        "CheckSpotFace tolerance-scaled comparison: inflated threshold makes "
        "0.12 appear < scaled threshold; spot misclassification triggered; "
        "companion 5 box faces close shell; "
        "defect IS on live CLOSED_SHELL traversal path"
    ),
)

# Face size just above spot threshold
W = 0.12   # width — slightly above 0.1 spot threshold
H = 0.5    # height of box (tall to make top face dominant)
...
# ── Top face z=H: 0.12×0.12 mm (the defect — vertex tol > face dimension) ────
...
f.write(_Path(__file__).parent.parent.parent / "step-examples" / "12-3c-faces" / "Tfa089.stp")
