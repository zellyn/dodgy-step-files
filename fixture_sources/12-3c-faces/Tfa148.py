...
Catalog claim: Wire area exactly equals min threshold; FixSmallAreaWire's
non-strict comparison removes or keeps depending on FP precision.
Trigger: face with inner wire of area exactly 0.00000250 mm² (5mm × 0.5mm
rectangle), at threshold boundary.

Mechanism: CLOSED_SHELL box (10×10×1 mm). The top face (z=1) has a standard
10×10 outer loop and an inner FACE_BOUND wire whose area is 0.00000250 mm²
(a 0.005 mm × 0.0005 mm rectangle). At the threshold boundary, non-strict
comparison (area <= threshold) removes the wire while strict (area <
threshold) keeps it; floating-point rounding near the threshold causes
non-deterministic behaviour. The inner wire is on the live traversal path.
...
f = StepFile(
    ...
    defect=(
        ...
        "top face (z=1) outer loop 10×10 plus inner FACE_BOUND wire of area "
        "exactly 0.00000250 mm² (0.005×0.0005 mm rectangle at (1,1,1)); "
        "FixSmallAreaWire non-strict comparison (area <= threshold) removes "
        "the wire; strict (area < threshold) keeps it; FP rounding at threshold "
        "boundary causes non-deterministic behaviour; "
...
# ── Top face: outer loop + threshold-boundary inner wire — THE DEFECT ────────
...
# Inner wire: 0.005 mm × 0.0005 mm rectangle at (1, 1, H)
# Area = 0.005 * 0.0005 = 0.0000025 mm² = 2.5e-6 mm² = threshold boundary
IW = 0.005     # inner wire width
IH_w = 0.0005  # inner wire height (so area = IW * IH_w = 2.5e-6)
p_i0 = f.cartesian_point((1.0,         1.0,          H))
p_i1 = f.cartesian_point((1.0 + IW,    1.0,          H))
p_i2 = f.cartesian_point((1.0 + IW,    1.0 + IH_w,   H))
p_i3 = f.cartesian_point((1.0,         1.0 + IH_w,   H))
...
ei12 = f.edge_curve(vi1, vi2, f.line(p_i1, f.vector(f.direction((0.0, 1.0, 0.0)), IH_w)))
ei23 = f.edge_curve(vi2, vi3, f.line(p_i2, f.vector(f.direction((-1.0, 0.0, 0.0)), IW)))
ei30 = f.edge_curve(vi3, vi0, f.line(p_i3, f.vector(f.direction((0.0, -1.0, 0.0)), IH_w)))
...
face_top = f.advanced_face([
    ...
    f.face_bound(inner_loop),    # threshold-boundary inner wire — THE DEFECT
], f.plane(ax_top))
...
f.write(_Path(__file__).parent.parent.parent / "step-examples" / "12-3c-faces" / "Tfa148.stp")
