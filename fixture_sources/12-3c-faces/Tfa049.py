"""Tfa049 — Pin diagnostic: classify smooth vs sharp vs stretched pin.

Catalog claim: An ADVANCED_FACE has a pin (singular tip), but pins come in
three morphologies that need different healing: smooth-tapered tip (no need
to act), sharp tip with discontinuous tangent, and stretched pin where another
vertex along the boundary could re-bound the face. The analyzer must
distinguish these.

Reproducer recipe (from catalog): A triangular ADVANCED_FACE with two
near-collinear long edges meeting at a tip (e.g., (0,0,0)->(10,0.001,0)->
(10,-0.001,0)->back); the angle is sharp enough to qualify as a sharp pin.

Mechanism: face[0] is the triangular pin ADVANCED_FACE on a PLANE.
  - v_tip: (0, 0, 0) — pin tip (zero width)
  - v_top: (10, 0.001, 0) — base top
  - v_bot: (10, -0.001, 0) — base bottom
  - aspect ratio ≈ 10/0.001 = 1e4 > 1e3 (pin criterion)
  - sliver_aspect_max_min > 1e3
...
The fixture is a triangular prism (extruded 1mm in Z):
  ...
  face[2]: top rectangular side (tip→top edge extruded)
  face[3]: bottom rectangular side (tip→bot edge extruded)
...
f = StepFile(
    ...
    defect=(
        ...
        "ShapeAnalysis_CheckSmallFace::CheckPin must classify as sharp pin; "
        "classifier returns 0=smooth/1=smooth-pin/2=sharp-or-stretched; "
        "prism extruded 1mm in Z to form closed shell; "
        "defect IS on live CLOSED_SHELL traversal path"
    ),
)

# Pin face vertices (z=0)
tip_x, tip_y = 0.0, 0.0
...
# Winding: tip0 → top0 → top1 → tip1 (outward normal perpendicular to long edge, pointing "up-right")
...
# Normal of this face: cross product of (top-tip) and Z
dx_tt = top_x - tip_x; dy_tt = top_y - tip_y
# outward normal = cross((dx_tt, dy_tt, 0), (0, 0, 1)) = (dy_tt, -dx_tt, 0)
...
# Winding: bot0 → tip0 → tip1 → bot1 (outward normal)
...
# Winding: top0 → bot0 → bot1 → top1 (outward normal = +X)
...
f.write(_Path(__file__).parent.parent.parent / "step-examples" / "12-3c-faces" / "Tfa049.stp")
