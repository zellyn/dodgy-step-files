...
Catalog claim: Face with inward-pointing surface normal (same_sense=.F.).
Healer detects via outer-wire winding direction and flips to outward
orientation. Planar square boundary, defect is orientation flag mismatch.

Mechanism: CLOSED_SHELL box (10×10×1 mm). One face (the top face at z=1) is
an ADVANCED_FACE with same_sense=.F. (inverted normal). The PLANE is defined
with axis pointing +Z but the face orientation flag is False, meaning the
surface normal effectively points inward (-Z). ShapeFix_Face::FixOrientation
detects this via the outer-wire winding direction (CCW from above = outer
face should point +Z) and flips the flag. The defect is the orientation
flag mismatch on the live CLOSED_SHELL path.
...
f = StepFile(
    catalog_id="Tfa081",
    defect=(
        "CLOSED_SHELL 10×10×1 mm box; top face (z=1) ADVANCED_FACE has same_sense=.F. "
        "(orientation flag inverted); PLANE axis points +Z but face flag negates it; "
        "FixOrientation: detects mismatch via outer-wire CCW winding; flips flag; "
        "5 companion faces (bottom + 4 sides) with correct orientation; "
...
# ── Bottom face z=0 (correct normal = -Z) ─────────────────────────────────────
...
# ── Top face z=H with REVERSED normal (same_sense=.F.) ───────────────────────
# Wire is CCW from above; PLANE axis points +Z; same_sense=False inverts the normal.
# This is the defect: the face normal effectively points inward.
...
# same_sense=False: reversed normal — the defect
face_top=f.advanced_face([f.face_outer_bound(top_loop)],f.plane(ax_top),same_sense=False)
...
f.write(_Path(__file__).parent.parent.parent / "step-examples" / "12-3c-faces" / "Tfa081.stp")
