"""Pmi115 — FLAT_PATTERN bend-radius below material thickness.

Catalog claim: AP242 flat_pattern manufacturability requires
bend_radius >= material_thickness. This fixture emits material_thickness=2.0 mm
and bend_radius=0.5 mm (radius < thickness). Receivers must warn
W_BEND_RADIUS_BELOW_THICKNESS; silent acceptance produces wrong K-factor.
...
f = StepFile(
    catalog_id="Pmi115",
    defect=(
        "flat_pattern material_thickness=2.0 mm and child bend_line "
        "bend_radius=0.5 mm; bend_radius < material_thickness violates "
        "sheet-metal manufacturability invariant"
    ),
)
...
# DEFECT: bend_radius < material_thickness
...
# material_thickness on the flat_pattern parent
...
# bend_radius = 0.5 mm — less than material_thickness (defect)
f._emit_raw(f"DIMENSIONAL_SIZE(#{bend_asp.eid},'bend_radius')")
f._emit_raw(
    "MEASURE_REPRESENTATION_ITEM('bend_radius',LENGTH_MEASURE(0.5),#9056)"
)
