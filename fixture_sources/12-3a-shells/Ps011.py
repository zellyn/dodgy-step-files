...
Catalog claim: a producer modelled a 10x10x10 box with a 4x4x4 inner cavity.
The export pipeline emitted a plain MANIFOLD_SOLID_BREP using only the outer
CLOSED_SHELL, dropping the void's CLOSED_SHELL. The kernel sees a fully-solid
1000-unit-cubed box; the producer expected a hollow body. Topology is fully valid.

Tier-3: face[0].area >= 90, n_edges_total >= 24,
        face[0].surface_type == "plane", face[5].surface_type == "plane"
...
f = StepFile(
    catalog_id="Ps011",
    defect=(
        "BREP_WITH_VOIDS downgraded to MANIFOLD_SOLID_BREP: outer 10x10x10 cube as "
        "CLOSED_SHELL; inner 4x4x4 cavity CLOSED_SHELL dropped by export pipeline; "
        "kernel sees fully-solid 1000 unit-cubed box; tier-3: detect void-loss by "
        "comparing MANIFOLD_SOLID_BREP vs BREP_WITH_VOIDS representation type; "
        "MANIFOLD_SOLID_BREP IS model entity — OCC yields shape(1)"
    ),
)

# Build 10x10x10 outer cube
pts = [
...
pl_bot = mk_plane(5, 5, 0,   0, 0,-1,  1, 0, 0)
pl_top = mk_plane(5, 5, 10,  0, 0, 1,  1, 0, 0)
pl_frt = mk_plane(5, 0, 5,   0,-1, 0,  1, 0, 0)
pl_bck = mk_plane(5,10, 5,   0, 1, 0,  1, 0, 0)
pl_lft = mk_plane(0, 5, 5,  -1, 0, 0,  0, 1, 0)
pl_rgt = mk_plane(10,5, 5,   1, 0, 0,  0, 1, 0)
...
# THE DEFECT: use MANIFOLD_SOLID_BREP instead of BREP_WITH_VOIDS — void is dropped
shell = f._emit_raw(f"CLOSED_SHELL('ps011_outer_shell',({face_refs}))")
msb   = f._emit_raw(f"MANIFOLD_SOLID_BREP('ps011_hollow_lost',#{shell.eid})")
