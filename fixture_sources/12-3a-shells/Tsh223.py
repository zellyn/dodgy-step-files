...
Catalog claim: GetShells() fails to partition all faces; remaining faces
discarded or orphaned. Incomplete decomposition breaks topology validation
and orientation analysis.

Mechanism IS the shell structure: TWO DISCONNECTED ADVANCED_FACEs (separate
edge-sets, no shared edges) in a single OPEN_SHELL IS the defect trigger.
GetShells() partitions by edge-connectivity; two isolated faces form two
separate components. The second component IS orphaned and discarded due to
incomplete decomposition — IS the extraction-loss defect that breaks topology
validation and orientation analysis.

This fixture targets the same code path as Tsh218 but with faces at different
Z offsets (Z=0 and Z=10) to produce a distinct geometry for oracle
differentiation (gmsh=shape(14)).
...
f = StepFile(
    ...
    defect=(
        ...
        "face A IS a unit-square at Z=0 — IS the first connected component; "
        "face B IS a unit-square at Z=10 (10 units away, no shared vertices or edges) — IS the second component; "
        "no topological connectivity between face A and face B — IS the disconnected shell topology; "
        "GetShells() traverses edge-connectivity from face A — IS the partitioning traversal; "
        ...
        "emit E_SHELLS_EXTRACTION_LOSS when faces remain after initial traversal"
    ),
)
...
# Face A — unit square at Z=0 (first connected component — IS the seed face)
...
# Face B — unit square at Z=10 (second component — IS the orphaned face)
pB00 = cp(0, 0, 10); vB00 = f.vertex_point(pB00)
pB10 = cp(1, 0, 10); vB10 = f.vertex_point(pB10)
pB11 = cp(1, 1, 10); vB11 = f.vertex_point(pB11)
pB01 = cp(0, 1, 10); vB01 = f.vertex_point(pB01)
...
# OPEN_SHELL: two disconnected faces at Z=0 and Z=10 — IS the GetShells() orphan trigger
