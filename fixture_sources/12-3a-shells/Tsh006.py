...
Catalog claim (kicad-packages3D): vendor-supplied component STEP models are
emitted as SHELL_BASED_SURFACE_MODEL (open shells) rather than
MANIFOLD_SOLID_BREP, even when topology actually closes.  PCB-CAD pipelines
that gate on MANIFOLD_SOLID_BREP drop these components silently on re-export.

Mechanism IS the shell structure: a 5-face open box is emitted as OPEN_SHELL
inside SHELL_BASED_SURFACE_MODEL — the same "wrong container" defect seen in
PCB vendor library models.  No CLOSED_SHELL or MANIFOLD_SOLID_BREP is present.
The absence of the solid entity IS wired into the shell/face topology.
...
f = StepFile(
    catalog_id="Tsh006",
    defect=(
        "Vendor PCB component model: 5-face OPEN_SHELL/SHELL_BASED_SURFACE_MODEL "
        "instead of MANIFOLD_SOLID_BREP; PCB-CAD pipeline gates on closed solid "
        "and silently drops; no MANIFOLD_SOLID_BREP IS the mechanism, wired into "
        "shell/face topology"
    ),
)

# ── Open box (5 faces, top absent): representative PCB component footprint ───
# This mimics a SOT-89-3 / TO-252-2 style body that ships as an open shell.
def pt(x, y, z):
...
# Unit-cube footprint; top face (z=1) absent — mimics vendor open-shell body
...
# ── DEFECT: 5-face body as OPEN_SHELL/SHELL_BASED_SURFACE_MODEL ───────────────
# No MANIFOLD_SOLID_BREP or CLOSED_SHELL — this IS the vendor-package defect.
# PCB pipeline gates on MANIFOLD_SOLID_BREP and silently drops the component.
open_sh = f.open_shell([face_bot, face_frt, face_bk, face_lft, face_rgt])
