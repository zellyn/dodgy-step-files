...
Catalog claim: A FACETED_BREP whose `outer` field references an OPEN_SHELL
rather than the schema-required CLOSED_SHELL.  Receivers abort with
"not supported", losing geometry entirely.

Mechanism IS the shell structure: a 6-face unit-cube topology is wired as
OPEN_SHELL and placed inside FACETED_BREP.  The schema violation IS the root
entity referencing the wrong shell type.
...
f = StepFile(
    catalog_id="Tsh002",
    defect=(
        "FACETED_BREP.outer references OPEN_SHELL instead of CLOSED_SHELL; "
        "schema violation IS wired into shell/face topology; OCCT silently accepts; "
        "strict receivers must reject"
    ),
)
...
# ── DEFECT: wrap the 6 faces in OPEN_SHELL then reference from FACETED_BREP ──
# Schema requires FACETED_BREP.outer to be CLOSED_SHELL.
...
fb = f._emit_raw(f"FACETED_BREP('tsh002_fb',#{open_sh.eid})")

f.add_product_chain(fb)
