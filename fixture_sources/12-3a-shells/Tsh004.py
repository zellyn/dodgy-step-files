...
Catalog claim: STEP intended to carry a solid arrives as OPEN_SHELL /
SHELL_BASED_SURFACE_MODEL because the writer never sewed faces into a
MANIFOLD_SOLID_BREP, or edge tolerances exceeded the receiver's sewing
tolerance.  FEA/CFD pipelines reject the body for meshing.

Mechanism IS the shell structure: a 5-face open box (cube minus its top face)
is emitted as OPEN_SHELL inside SHELL_BASED_SURFACE_MODEL — representing a
surface body that the writer should have closed/sewed into a solid.  The gap
(missing top face) IS wired into the shell/face topology.
...
f = StepFile(
    catalog_id="Tsh004",
    defect=(
        "Open box (5 faces, top missing): OPEN_SHELL/SHELL_BASED_SURFACE_MODEL "
        "instead of sewed solid; gap IS wired into shell/face topology; "
        "FEA/CFD pipeline rejects for meshing; heal requires sewing missing face"
    ),
)

# ── Open box: 5 faces of a unit cube, top (z=1) intentionally absent ────────
def pt(x, y, z):
...
# ── DEFECT: 5-face open box (no top) as OPEN_SHELL/SHELL_BASED_SURFACE_MODEL ──
# Top face (z=1) is absent — this gap IS the FEA-pipeline defect wired into
# the shell/face topology.  No MANIFOLD_SOLID_BREP is emitted.
