...
Catalog claim: One or more faces in a closed shell have their `same_sense` flag
inverted relative to the outward direction of the solid; renderer shows
inside-out shading on that face; simulation BCs break.

Mechanism IS the shell structure: a 6-face CLOSED_SHELL / MANIFOLD_SOLID_BREP
unit-cube is built with five faces correctly oriented (same_sense=True) and
exactly ONE face — the top face — with same_sense=False.  The single flipped
face IS wired into the shell/face topology; its normal points inward while the
remaining five are correct.
...
f = StepFile(
    catalog_id="Tsh010",
    defect=(
        "MANIFOLD_SOLID_BREP 6-face cube: top ADVANCED_FACE has same_sense=False so its "
        "normal points inward while the other 5 faces are correctly outward; single-face "
        "reversal IS wired into shell/face topology; strict receivers must propagate "
        "orientation and flip or reject"
    ),
)

# ── Unit-cube: 6 faces, top face flipped ─────────────────────────────────────
# Five faces have same_sense=True (outward).  Top face (z=1) has same_sense=False
# (inward) — the single reversed-face defect IS the mechanism.
...
# Bottom z=0, normal (0,0,-1) — same_sense=True (correct outward)
...
# DEFECT: same_sense=False reverses this face's normal to (0,0,-1) — points inward
face_top = f.advanced_face([f.face_outer_bound(lp_top)], f.plane(ax_top), same_sense=False)

# Front y=0, normal (0,-1,0) — same_sense=True (correct)
...
# Back y=1, normal (0,1,0) — same_sense=True (correct)
...
face_bk = f.advanced_face([f.face_outer_bound(lp_bk)], f.plane(ax_bk), same_sense=True)

# Left x=0, normal (-1,0,0) — same_sense=True (correct)
...
# Right x=1, normal (1,0,0) — same_sense=True (correct)
...
face_rgt = f.advanced_face([f.face_outer_bound(lp_rgt)], f.plane(ax_rgt), same_sense=True)

# ── DEFECT: single face (top) flipped IS wired into the CLOSED_SHELL ──────────
# face_top has same_sense=False; all others are True.  The single-face
# reversal IS the mechanism — wired into shell/face topology.
