...
Catalog defect: PLANE host; 3D LINE from (0,0,0) to (10,0,0); PCURVE B-spline.
Defect: midspan 3D curve point (5,0,0) maps to PCURVE point (5.1, 0) —
0.1 mm divergence exceeding edge tolerance 0.001 mm. CheckCurve3dWithPCurve
sample-point inspection detects the mismatch and flags the edge for healing.

Fixture: rectangular PLANE face. Bottom edge has correct 3D LINE from
(0,0,0) to (10,0,0) but its PCURVE is a quadratic B-spline whose middle
control point is displaced to U=5.1 instead of U=5.0, causing the midspan
parametric sample to map to x=5.1 on the surface instead of x=5.0.
"""
...
f = StepFile(
    catalog_id="Gp041",
    defect=(
        "PLANE face bottom edge: 3D LINE (0,0,0)->(10,0,0); PCURVE B-spline "
        "midspan control point at U=5.1 instead of U=5.0; 0.1 mm sample-point "
        "divergence far exceeds edge tolerance 0.001 mm; "
        "CheckCurve3dWithPCurve flags for healing"
    ),
)
...
# THE DEFECT: 3D LINE correct; PCURVE B-spline has midspan offset.
# 3D LINE: (0,0,0) -> (10,0,0)
d_x   = f.direction((1.0, 0.0, 0.0))
vec_x = f.vector(d_x, 10.0)
line_bot_3d = f.line(p00, vec_x)

# PCURVE: quadratic B-spline in UV space.
# degree=2, 3 control points: (0,0), (5.1,0), (10,0)   <- DEFECT: midpoint at 5.1 not 5.0
# knot mults [3,3], knots [0.0, 1.0]  => sum mults = 6 = 3 poles + 2 + 1 = 6. OK.
cp0 = f.cartesian_point((0.0,  0.0))
cp1 = f.cartesian_point((5.1,  0.0))   # DEFECT: should be (5.0, 0.0)
cp2 = f.cartesian_point((10.0, 0.0))
