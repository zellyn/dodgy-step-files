"""Gp179 — 3D-curve endpoint disagrees with vertex position beyond
tolerance; pcurve is trustworthy (mirror-image of Gp064's pcurve-side case).

Work packet D1, item `tkshh-edge-curve-inconsistent-with-vertex-removed`
(PARTIAL, missing 1 of 5): "An edge whose 3D curve endpoint disagrees with
its vertex position beyond tolerance while its PCURVE is trustworthy -- the
3D-curve-side removal case (Gp064 covers the pcurve-side; build the mirror
image)." problem_id `tkshh-edge-curve-inconsistent-with-vertex-removed`
(ShapeFix_Edge::FixRemoveCurve3d, using ShapeAnalysis_Edge::
CheckVerticesWithCurve3d) is the mirror of ShapeFix_Edge::FixRemovePCurve
(Gp064's mechanism, ShapeAnalysis_Edge::CheckVerticesWithPCurve): rather than
repairing a stale/wrong representation in place, the healer discards it and
rebuilds it fresh from the other (trusted) representation -- here the 3D
curve is the stale one, the pcurve is trusted.

Geometry: the standard 4-edge rectangular PLANE face used throughout this
section (cf. Gp034/Gp045/Gp175/Gp176). Only the bottom edge (A=(0,0,0) ->
B=(10,0,0)) is defective: its 3D LINE is shifted +0.5 in Y (start=(0,0.5,0),
direction (1,0,0)) so neither endpoint of the *curve* passes anywhere near
the actual VERTEX_POINT positions -- the closest point on the 3D line to
vertex A is (0,0.5,0), 0.5 units away (500,000x the fixture's 1e-7
tolerance); same for vertex B. The edge's PCURVE, by contrast, is the
correct straight line (0,0)->(10,0) in the plane's UV space, matching the
true rectangle geometry exactly. A correct healer must discard the bogus 3D
curve and rebuild it by lifting the trustworthy pcurve through the PLANE.
"""
from step_corpus.step_builder import StepFile

f = StepFile(
    catalog_id="Gp179",
    defect=(
        "Bottom edge of a standard 4-edge rectangular PLANE face: 3D LINE "
        "is shifted +0.5 in Y (start=(0,0.5,0)) so NEITHER endpoint of the "
        "curve is within tolerance of its VERTEX_POINT (A=(0,0,0), "
        "B=(10,0,0) -- both 0.5 units off). PCURVE is the correct, "
        "trustworthy straight line (0,0)->(10,0) in UV. Mirror-image of "
        "Gp064's pcurve-side FixRemovePCurve case: here FixRemoveCurve3d "
        "should discard the 3D curve and rebuild it from the pcurve."
    ),
)

# Flat PLANE at Z=0 as the host surface.
p_orig = f.cartesian_point((0.0, 0.0, 0.0))
p_norm = f.direction((0.0, 0.0, 1.0))
p_ref = f.direction((1.0, 0.0, 0.0))
p_axis = f.axis2_placement_3d(p_orig, p_norm, p_ref)
plane = f.plane(p_axis)

prc = f._emit_raw(
    "(GEOMETRIC_REPRESENTATION_CONTEXT(2)PARAMETRIC_REPRESENTATION_CONTEXT()"
    "REPRESENTATION_CONTEXT('UV','2D'))"
)

# Rectangle corners (Z=0): A=(0,0,0) B=(10,0,0) C=(10,5,0) D=(0,5,0)
p_a = f.cartesian_point((0.0, 0.0, 0.0))
p_b = f.cartesian_point((10.0, 0.0, 0.0))
p_c = f.cartesian_point((10.0, 5.0, 0.0))
p_d = f.cartesian_point((0.0, 5.0, 0.0))
v_a = f.vertex_point(p_a)
v_b = f.vertex_point(p_b)
v_c = f.vertex_point(p_c)
v_d = f.vertex_point(p_d)

# ---- Bottom edge A->B: THE DEFECT. 3D LINE shifted +0.5 in Y; correct pcurve. ----
p_shifted_start = f.cartesian_point((0.0, 0.5, 0.0))
d_ab = f.direction((1.0, 0.0, 0.0))
v_ab = f.vector(d_ab, 10.0)
line_ab_wrong = f.line(p_shifted_start, v_ab)

pc_bot_start = f.cartesian_point((0.0, 0.0))
pc_bot_dir = f.direction((1.0, 0.0))
pc_bot_vec = f.vector(pc_bot_dir, 10.0)
pc_bot_line = f.line(pc_bot_start, pc_bot_vec)
pc_bot_def = f._emit_raw(
    f"DEFINITIONAL_REPRESENTATION('pc_bot_correct',(#{pc_bot_line.eid}),#{prc.eid})"
)
pcurve_bot = f._emit_raw(f"PCURVE('bot_pc_trustworthy',#{plane.eid},#{pc_bot_def.eid})")
sc_bot = f._emit_raw(
    f"SURFACE_CURVE('bottom_curve3d_wrong',#{line_ab_wrong.eid},"
    f"(#{pcurve_bot.eid}),.PCURVE_S1.)"
)
edge_bot = f._emit_raw(
    f"EDGE_CURVE('bottom_edge',#{v_a.eid},#{v_b.eid},#{sc_bot.eid},.T.)"
)

# ---- Right edge B->C (healthy) ----
d_bc = f.direction((0.0, 1.0, 0.0))
v_bc = f.vector(d_bc, 5.0)
line_bc = f.line(p_b, v_bc)
pc_right_start = f.cartesian_point((10.0, 0.0))
pc_right_dir = f.direction((0.0, 1.0))
pc_right_vec = f.vector(pc_right_dir, 5.0)
pc_right_line = f.line(pc_right_start, pc_right_vec)
pc_right_def = f._emit_raw(
    f"DEFINITIONAL_REPRESENTATION('pc_right',(#{pc_right_line.eid}),#{prc.eid})"
)
pcurve_right = f._emit_raw(f"PCURVE('right_pc',#{plane.eid},#{pc_right_def.eid})")
sc_right = f._emit_raw(
    f"SURFACE_CURVE('right_edge',#{line_bc.eid},(#{pcurve_right.eid}),.PCURVE_S1.)"
)
edge_right = f._emit_raw(
    f"EDGE_CURVE('right_edge',#{v_b.eid},#{v_c.eid},#{sc_right.eid},.T.)"
)

# ---- Top edge D->C, traversed reversed (healthy) ----
d_dc = f.direction((1.0, 0.0, 0.0))
v_dc = f.vector(d_dc, 10.0)
line_dc = f.line(p_d, v_dc)
pc_top_start = f.cartesian_point((0.0, 5.0))
pc_top_dir = f.direction((1.0, 0.0))
pc_top_vec = f.vector(pc_top_dir, 10.0)
pc_top_line = f.line(pc_top_start, pc_top_vec)
pc_top_def = f._emit_raw(
    f"DEFINITIONAL_REPRESENTATION('pc_top',(#{pc_top_line.eid}),#{prc.eid})"
)
pcurve_top = f._emit_raw(f"PCURVE('top_pc',#{plane.eid},#{pc_top_def.eid})")
sc_top = f._emit_raw(
    f"SURFACE_CURVE('top_edge',#{line_dc.eid},(#{pcurve_top.eid}),.PCURVE_S1.)"
)
edge_top = f._emit_raw(
    f"EDGE_CURVE('top_edge',#{v_d.eid},#{v_c.eid},#{sc_top.eid},.T.)"
)

# ---- Left edge D->A (healthy) ----
d_da = f.direction((0.0, -1.0, 0.0))
v_da = f.vector(d_da, 5.0)
line_da = f.line(p_d, v_da)
pc_left_start = f.cartesian_point((0.0, 5.0))
pc_left_dir = f.direction((0.0, -1.0))
pc_left_vec = f.vector(pc_left_dir, 5.0)
pc_left_line = f.line(pc_left_start, pc_left_vec)
pc_left_def = f._emit_raw(
    f"DEFINITIONAL_REPRESENTATION('pc_left',(#{pc_left_line.eid}),#{prc.eid})"
)
pcurve_left = f._emit_raw(f"PCURVE('left_pc',#{plane.eid},#{pc_left_def.eid})")
sc_left = f._emit_raw(
    f"SURFACE_CURVE('left_edge',#{line_da.eid},(#{pcurve_left.eid}),.PCURVE_S1.)"
)
edge_left = f._emit_raw(
    f"EDGE_CURVE('left_edge',#{v_d.eid},#{v_a.eid},#{sc_left.eid},.T.)"
)

# Loop: bot(fwd A->B, DEFECT) -> right(fwd B->C) -> top(rev C->D) -> left(fwd D->A)
loop = f.edge_loop([
    f.oriented_edge(edge_bot, True),
    f.oriented_edge(edge_right, True),
    f.oriented_edge(edge_top, False),
    f.oriented_edge(edge_left, True),
])
face = f.advanced_face([f.face_outer_bound(loop)], plane)
shell = f.open_shell([face])
sbsm = f.shell_based_surface_model([shell])
f.add_product_chain(sbsm)
