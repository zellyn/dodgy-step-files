...
Catalog claim: OCAF's modification-delta mechanism for array-style attributes
(Active Data Mesh Attribute) skips a destructor on the previous state;
long-running suites that perform thousands of modify/commit cycles accumulate
memory. A single STEP fixture cannot demonstrate the leak directly — the defect
is cross-transaction state accumulation. The fixture provides a multi-face solid
so that a test driver can open/mutate/commit it thousands of times and observe
monotonic RSS growth.

Byte assertion: (none enforced by catalog; shape(1) expected)
Tier-3: load == "ok", n_edges_total >= 3, n_vertices_total >= 24, brepcheck.valid == True
...
f = StepFile(
    catalog_id="Pf020",
    defect=(
        "OCAF modification-delta leak: Backup/Delta lifetime in OCAF must be bounded; "
        "coerce to release after commit; this fixture is a multi-face solid used by a "
        "test driver that performs thousands of modify/commit cycles; observe RSS growth "
        "proportional to iteration count; "
        "MANIFOLD_SOLID_BREP IS model entity — OCC yields shape(1)"
    ),
)

# Unit cube: 8 vertices, 12 edges, 6 faces, 1 MANIFOLD_SOLID_BREP
pts = [
...
shell = f._emit_raw(f"CLOSED_SHELL('pf020_ocaf_leak_shell',({face_refs}))")
msb   = f._emit_raw(f"MANIFOLD_SOLID_BREP('pf020_ocaf_leak_solid',#{shell.eid})")
